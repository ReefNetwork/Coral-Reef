            ...
            switch (true) {
                case $limit || ($firstRand <= 5) || ($isFirst && ($firstRand <= 10)): // 0.5% (まだ1回も引いてない人は1%)
                    switch (mt_rand(1, 10)) {
                        ...
                        case 2:
                        case 3:
                            $item = Summer2022ReefItems::getItem($xuid, Summer2022ReefItems::PICKAXE);
                            break;
                        case 4:
                        case 5:
                        case 6:
                            $item = Summer2022ReefItems::getItem($xuid, Summer2022ReefItems::SHOVEL);
                            break;
                        case 7:
                        case 8:
                        case 9:
                            $item = Summer2022ReefItems::getItem($xuid, Summer2022ReefItems::AXE);
                            break;
                        case 10:
                            $item = Summer2022ReefItems::getItem($xuid, Summer2022ReefItems::HOE);
                            break;
                        default:
                ...
                default:
                    $items = [VanillaItems::WATER_POTION()->setCount(1), VanillaItems::STEAK()->setCount(4),
                        VanillaItems::APPLE()->setCount(8), VanillaItems::SWEET_BERRIES()->setCount(4), VanillaItems::GOLDEN_APPLE()];
                    GatyaManager::gatyaProcess($repo, SQLConst::LOG_GATYA_SUMMER_2022, $p, SQLConst::TICKETS_SUMMER_2022, 1,
                        $items[array_rand($items)],
                        "normal", TextFormat::GOLD . "ノーマル" . TextFormat::DARK_GRAY . "[50%]" . TextFormat::RESET, false, $func);
                    break;
            }
        }, function (SqlError $error) use ($p) {
            $p->sendMessage("エラーが発生しました");
            Server::getInstance()->getLogger()->error("[Summer2022GatyaCheckLimit] " . $p->getName() . "さんの処理中に" . $error->getErrorMessage());
        });
    }
}
