...
class SteamPunk
{
    ...
    static function gatya(SQLRepository $repo, Player $p, int $number = 1): void
    {
        if ($number <= 0) return;
        $xuid = $p->getXuid();
        $repo->getLog($xuid, SQLConst::LOG_GATYA_STEAM_PUNK, function (array $rows) use ($repo, $number, $p, $xuid) {
            ...
            switch (true) {
                case ($firstRand <= 5) || $isLimit:// 0.5% or 天井
                    switch (mt_rand(1, 10)) {
                        ...
                        case 2:
                        case 3:
                            $item = SteamPunkItems::getItem($xuid, SteamPunkItems::PICKAXE);
                            break;
                        case 4:
                        case 5:
                        case 6:
                            $item = SteamPunkItems::getItem($xuid, SteamPunkItems::SHOVEL);
                            break;
                        case 7:
                        case 8:
                        case 9:
                            $item = SteamPunkItems::getItem($xuid, SteamPunkItems::AXE);
                            break;
                        case 10:
                            $item = SteamPunkItems::getItem($xuid, SteamPunkItems::HOE);
                            break;
                        default:
