...
class ItemShopManageForm
{
                                   ...
                                   int       $dayLimit = 0): CustomForm
    {
        ...
        return (new ClosureCustomForm(function (Player $p) use ($dayLimitElement, $orderElement, $pos, $store, $amountElement, $typeElement): void {
            ...
            $store->createShop(new ItemShop($pos, $order, ["type" => $payment, "amount" => $amount], $dayLimit, []));
            $p->sendMessage("ショップを作成しました");
        }))->setTitle("ItemShop Edit")->addElements($orderElement, $typeElement, $amountElement, $dayLimitElement);
    }
}
